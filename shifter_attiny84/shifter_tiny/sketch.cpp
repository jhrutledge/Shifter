...
// PINOUT
 const int bcdPins[] = {0,1,2}; // {0,1,2} pins to decoder
 const int solPins[] = {7,10}; //{sol1,sol2}
 const int TCPins[] = {10,11}; //{DP,sol3} direct wired pin to DP
 const int upPin = A1;       //
 const int dnPin = A2;       //
 const int brkPin = A3;      //
 const int dPin = A4;
...
// gear datastructure
struct gear{
  int bcd[];	// states for BCD display
  int sol[];	// states for solenoids
};
...
// VARS
 ...
 const button upButton {upPin,LOW,LOW,LOW,0};
 const button dnButton {dnPin,LOW,LOW,LOW,0};
 const button brkButton {brkPin,LOW,LOW,LOW,0};
